 ...
 * @author C2079
 */
@ManagedBean(name = "repairTransferStatisticsManagedBean")
@SessionScoped
public class RepairTransferStatisticsManagedBean extends FormMultiBean<EquipmentRepair, EquipmentRepairHis> {
    ...
    public void init() {
        ...
        company = str;//初始化公司别
        String comSql = " H.company= '" + company[0] + "'";//根据公司别查询数据的SQL条件
        detailList = equipmentRepairHisBean.getRepairTransferStatisticsList(simpleDateFormat.format(queryDateBegin), simpleDateFormat.format(queryDateEnd), comSql);
    }
...
//导出界面的EXCEL数据处理
    ...
    public void print() throws ParseException {
        ...
        fileName = "维修转派统计表" + BaseLib.formatDate("yyyyMMddHHmmss", BaseLib.getDate()) + ".xls";
        ...
        cellTitle.setCellValue("维修转单统计表");
        ...
        for (Object[] eq : list) {
            ...
            cell5.setCellValue(sdf.format(sdf.parse(eq[5].toString())));
    ...
    private String[] getInventoryTitle() {
        return new String[]{"原辅助维修人", "被转单负责人", "报修单号", "资产编号", "设备名称", "故障发生日期", "转单事由"};
    }
